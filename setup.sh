...
if [ -f "python-api/requirements.txt" ]; then
    ...
    warn "python-api/requirements.txt introuvable — étape ignorée"
fi
...
if [ -f "node-client/package.json" ]; then
    ...
    warn "node-client/package.json introuvable — étape ignorée"
fi
...
if [ ! -f "$LOG" ]; then
    ...
    if [ "$NB_CRIT" -gt 0 ]; then
        echo -e "${ROUGE}  ALERTE : $NB_CRIT incident(s) critique(s) détecté(s) :${RESET}"
